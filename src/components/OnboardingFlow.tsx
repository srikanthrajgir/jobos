...
import { submitResumeBuilder, parseUploadedResume, submitCareerPreferences, completeOnboarding } from '@/app/actions/onboarding';

export default function OnboardingFlow({ initialState }: { initialState: any }) {
  ...
  const handleUploadClick = async () => {
    ...
    setTimeout(async () => {
      // Mock resume ID
      await parseUploadedResume("mock-id");
  ...
  const handleBuilderClick = async () => {
    setLoading(true);
    await submitResumeBuilder({ name: "User", summary: "Built Resume" });
    setStep('career_preferences_required');
    setLoading(false);
  };
          ...
          {step === 'resume_required' && (
            <motion.div key="resume" initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} exit={{opacity:0, x:-20}} className="w-full max-w-md">
              ...
              <div className="space-y-4">
                ...
                >
                  <div className="text-left">
                    <h3 className="font-bold text-lg text-text-heading">Build a Résumé</h3>
                    <p className="text-sm text-text-muted mt-1 font-medium">Don't have one? Build one step by step.</p>
                  </div>
